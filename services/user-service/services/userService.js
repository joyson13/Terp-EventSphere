...
class UserService {
  ...
  async register(userData) {
    ...
    // Validate .umd.edu email domain
    const emailDomain = userData.email.toLowerCase().split('@')[1];
    if (emailDomain !== 'umd.edu') {
      throw new Error('Registration is only allowed with @umd.edu email addresses');
    }
    ...
    const userDataWithName = {
      ...userData,
      name: userData.name || userData.email.split('@')[0] // Default name from email if not provided
    };

    return await userRepository.create(userDataWithName);
  }
